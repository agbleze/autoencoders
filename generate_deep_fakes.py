...
# %%  define convolution and upscaling, reshaping
def _ConvLayer(input_features, output_features):
    return nn.Sequential(nn.Conv2d(input_features, output_features, kernel_size=5, stride=2, padding=2),
                         nn.LeakyReLU(0.1, inplace=True)
                         )
    
def _UpScale(input_features, output_features):
    return nn.Sequential(nn.ConvTranspose2d(input_features, output_features, kernel_size=2, stride=2, padding=0),
                         nn.LeakyReLU(0.1, inplace=True)
                         )
...
optimizers = optim.Adam([{"params": model.encoder.parameters()},
                        ...
                        lr=5e-5, betas=(0.5, 0.999)), optim.Adam([{"params": model.encoder.parameters()}, 
